...
\begin{table}
	...
	\hline
	\code{rand(r,c,m,s,l)}& Creates matrix of size $(r,c)$ with $l$ non-zero elements, distributed uniformly. The non-zero cells are drawn from Gaussian distribution with mean $m$ and standard deviation $s$.& $double\times double\times double \times double \times double \rightarrow matrix[double]$\\
	\hline

...
The first approach is based on replicating rows and columns of the operands and is called replication based matrix multplication (RMM).
...
A block row will be denoted by a single subscripted index and a column row by a single superscripted index.
...
One of the optimizations concerns the join function.
...
The resulting exeuction plan under these assumptions is equivalent to the RMM.

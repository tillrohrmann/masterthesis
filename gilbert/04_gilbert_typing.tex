...
Therefore, a type dictionary is maintained where types of expressions, which already have been seen while parsing the code, are stored.
...
Even though these side effects contradict the purity of logical deduction rules, it allows us to express the algorithm concisely.
...
The subsequent function call \code{f(1.0)} in line $2$ will, therefore, throw a typing exception, because a scalar argument cannot be applied to a function having a matrix parameter.

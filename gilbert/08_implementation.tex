...
We have chosen this library as our math backend because it is clean, very reliable and extremely powerful.

\section{Execution Engines}

From the very beginning onwards, Gilbert was intended as a general purpose frontend for different runtime systems.
...
For example, an executor using a messsage passing system for the parallel execution is easily conceivable.
We refrained from it, though, because of its error-prone implementation.
We will describe the Stratosphere and Spark parallel executors and the problems we encounterd while implementing Gilbert in the following subsections.
...
Furthermore, we could reuse the distributed matrices defined for Stratosphere and the math backend.
